...
int escolhahab, escolhaacao, inimigosort, comb = 1, round = 1;
...
while(comb == 1){
    ...
    if(inimigo.vida <= 0){ 
        ...
        if(alturaPilhapok(&pokepilha[1]) == 0){
            ...
            embaralhar(pokedex,15);
            for (int i = 0; i < 15; i++) {
                if (!pokemonNaPilha(&pokepilha[0], pokedex[i])) {           //todos os pokemons que você nao tem vao para a pilha de inimigos
                    pushpok(&pokepilha[1], pokedex[i]);
                        }
                                }
        }
        ...
        round ++;  
        printf("%s ficou mais forte!", aliado.nome);
        level_up(&aliado);
        }         
